...
class DashBoardReport:
    ...
    def __init__(self, helper: AnalysisHelper, analysis_engine: AnalysisEngine):
        self.helper = helper
    ...
    # DASHBOARD CREATION
    def run_dashboard(self, tickers: list = None, asset_classes: list = None, period: str = "1y", volatility_window: int = 30, save_file: bool = True) -> str:
        """
        ...
        Returns:
            Dashboard text.
        """
        selected_tickers = self.helper.get_tickers_for_analysis(tickers, asset_classes)

        if not selected_tickers:
            logger.warning("No tickers selected.")
            return ""

        stats = self.analysis_engine.compute_insights(tickers=selected_tickers, period=period, volatility_window=volatility_window)
        dashboard = self.format_dashboard(stats, selected_tickers, period)
...
if __name__ == "__main__":
    ...
    dashboard = DashBoardReport(helper=helper, analysis_engine=analysis_engine)
    
    dashboard.run_dashboard(tickers=["AAPL", "BND", "SPY"], period="1y")

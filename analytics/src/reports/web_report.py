...
class WebReport:
    ...
    def generate_web_insights(summary_stats: pd.DataFrame) -> dict:
        ...
        for column in ['Annualized Return (%)', 'Annualized Volatility (%)', 'Sharpe Ratio']:
        ...
        if 'Annualized Volatility (%)' in stats.columns:
            ...
            if not valid.empty:
                ...
                ranking = valid[['Ticker', 'Annualized Volatility (%)']].sort_values('Annualized Volatility (%)', ascending=False)
    ...
    # WEB REPORT CREATION
    def create_web_data(self, summary_stats: pd.DataFrame, tickers: list, period: str) -> dict:
        """
        ...
        return {
            ...
            'insights': self.generate_web_insights(summary_stats),
            'charts': chart_data
        }
    ...
    def save_web_data(web_data: dict, tickers: list, period: str) -> Path:
        ...
        with open(output_path, 'w', encoding='utf-8') as file:
            json.dump(web_data, file, indent=4, default=str)
    ...
    def generate_web_report(
        ...
        try:
            analysis_tickers = self.helper.get_tickers_for_analysis(tickers=tickers, asset_classes=asset_classes)
